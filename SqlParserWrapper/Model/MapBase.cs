...
namespace SqlParserWrapper.Model
{
    ...
    public class MapBase
    {
        ...
        protected Dictionary<string, TableEntity> _tables = new Dictionary<string, TableEntity>();

        protected List<string> mappings = new List<string>();
        ...
        protected string BuildOutput(ExpressionEntity source)
        {
            string output = string.Empty;
            ...
            //}
            //else if (source.SearchedCase is not null)
            //{
            //    output += GetSearchedCaseVal(source.SearchedCase);
            //}
            //else if (source.IIf is not null)
            //{
            //    output += BuildIIf(source.IIf);
            //}
        ...
        protected string GetIdentifer(List<IdentiferEntity> multiPart)
        {
            ...
            foreach (var item in multiPart)
            {
                if (count == index)
                {
                    if (_tables.ContainsKey(item.GetValue().ToLower()))
                    {
                        string aliasToReplace = item.GetValue();
                        string fullName = _tables[item.GetValue().ToLower()].TableReference.NamedTable.SchemaObject.FullName;

                        // if the table is referenced multiple times we need to stick with the alias

                        var x = (from t in _tables.Keys
                                where _tables[t.ToLower()].TableReference.NamedTable.SchemaObject.FullName == fullName
                                select t).ToList();

                        if (x.Count > 1)
                        {
                            // use alias rather than full name
                            output += item.GetValue();
                        }
                        else
                        {
                            output += item.GetValue().Replace(aliasToReplace, fullName);
                        }
                    }
                    else
                    {
